...
def vinyl_add():
    ...
    if request.method == 'GET':
        ...
        cur.execute("""INSERT into media(type,vendor,product,supplier,roll_width,roll_length,cost) VALUES (%s,%s,%s,%s,%s,%s) RETURNING *;""", ('vinyl',vendor,product,supplier,rollwidth,rolllength,cost))
        rows = cur.fetchall()
        conn.commit()
        cur.close
        conn.close()
